...
CALL is not an issued currency. CALL is the only native asset on the CALL Ledger and is required in order to conduct transactions on the CALL Ledger. CALL is counterparty free, meaning that when someone holds CALL, they are not holding a liability, they are holding the actual currency, CALL. Due to this fact, _**<u>CALL CANNOT be frozen by any entity or individual</u>**_.

All non-CALL currencies can be represented in the CALL Ledger as issued currencies. These issued currencies (sometimes called "issuances" or "IOUs") are tracked in accounting relationships, called "trust lines," between addresses. Issued currencies are typically considered as liabilities from one perspective and assets from the other, so the balance of a trust line is negative or positive depending on which side you view it from. Any address may freely issue (non-CALL) currencies, limited only by how much other addresses are willing to hold.

In certain cases, to meet regulatory requirements, or while investigating suspicious activity, an exchange or gateway may want to quickly freeze non-CALL issued currency balances.

**Tip:** No one can freeze CALL.
...
The freeze feature only applies to issued currencies. Because no party has a privileged place in the CALL Ledger, the freeze feature cannot prevent a counterparty from conducting transactions in CALL or funds issued by other counterparties. No one, not even Call, can freeze CALL.
...
Reminder: Trust lines do not hold CALL. CALL cannot be frozen.

A financial institution can freeze the trust line linking it to a counterparty if that counterparty shows suspicious activity or violates the financial institution's terms of use. The financial institution should also freeze the counterparty in any other systems the financial institution operates that are connected to the CALL Ledger. (Otherwise, an address might still be able to engage in undesired activity by sending payments through the financial institution.)
...
Reminder: CALL already cannot be frozen. The No Freeze feature only applies to other currencies issued in the CALL Ledger.
...
|----------------------|--------|-------------|
| Account              | String | The CALL Ledger address to enable or disable the freeze. |
| TransactionType      | String | `TrustSet` |
| LimitAmount          | Object | Object defining the trust line to freeze. |
| LimitAmount.currency | String | Currency of the trust line (cannot be CALL) |
| LimitAmount.issuer   | String | The CALL Ledger address of the counterparty to freeze |
...
Example of submitting a TrustSet transaction to enable an individual freeze using the [WebSocket API](get-started-with-the-calld-api.html#websocket-api):
...
To enable or disable Individual Freeze on a specific trust line, prepare a *Trustline* transaction using the [prepareTrustline](callapi-reference.html#preparetrustline) method. The fields of the `trustline` parameter should be set as follows:

| Field        | Value  | Description |
|--------------|--------|-------------|
| currency     | String | The [currency](callapi-reference.html#currency) of the trust line to freeze (cannot be CALL) |
| counterparty | String | The [CALL Ledger address](callapi-reference.html#address) of the counterparty |
...
The rest of the [transaction flow](callapi-reference.html#transaction-flow) is the same as any other transaction.
...
Example of submitting an AccountSet transaction to enable Global Freeze using the [WebSocket API](get-started-with-the-calld-api.html#websocket-api):
...
To enable or disable Global Freeze on an address, prepare a **Settings** transaction using the [prepareSettings](callapi-reference.html#preparesettings) method. The `settings` parameter should be an object set as follows:
...
Example of submitting an AccountSet transaction to enable No Freeze using the [WebSocket API](get-started-with-the-calld-api.html#websocket-api):
...
To enable No Freeze on an address, prepare a **Settings** transaction using the [prepareSettings](callapi-reference.html#preparesettings) method. Once enabled, you cannot disable No Freeze. The `settings` parameter should be an object set as follows:

| Field    | Value   | Description |
|----------|---------|-------------|
| noFreeze | Boolean | `true`      |

You must [sign](callapi-reference.html#sign) this transaction using the master key. The rest of the [transaction flow](callapi-reference.html#transaction-flow) is the same as any other transaction.
...
| Field    | Value   | Description |
|----------|---------|-------------|
| account  | String  | The CALL Ledger address of the issuer |
| peer     | String  | The CALL Ledger address of the counterparty |
...
The field `"freeze": true` indicates that cf1BiGeXwwQoi8Z2ueFYTEXSwuJYfV2Jpn has enabled Individual Freeze on the USD trust line to csA2LpzuawewSBQXkiju3YQTMzW13pAAdW. The lack of a field `"freeze_peer": true` indicates that the counterparty has _not_ frozen the trust line.


### Using CallAPI

To see if a trust line has an Individual Freeze enabled, use the [`getTrustlines` method](callapi-reference.html#gettrustlines) with the following parameters:

| Field         | Value   | Description |
|---------------|---------|-------------|
| address       | String  | The CALL Ledger address of the issuer |
| options.counterparty  | String  | The CALL Ledger address of the counterparty |
...
| Field    | Value   | Description |
|----------|---------|-------------|
| account  | String  | The CALL Ledger address of the issuing address |
| ledger\_index | String | Use `validated` to get the most recently validated information. |
...
In the above example, the `Flags` value is 12582912. This indicates that has the following flags enabled: lsfGlobalFreeze, lsfDefaultCall, as demonstrated by the following JavaScript code:
...
To see if an address has enabled Global Freeze, No Freeze, or both, use the [`getSettings` method](callapi-reference.html#getsettings) with the following parameters:

| Field         | Value   | Description |
|---------------|---------|-------------|
| address       | String  | The CALL Ledger address of the issuing address |
...
* [Freeze Code Samples](https://github.com/callchain/call-dev-portal/tree/master/content/_code-samples/freeze)
