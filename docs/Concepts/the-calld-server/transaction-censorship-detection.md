...
The CALL Ledger is designed to be [censorship resistant](call-ledger-overview.html#censorship-resistant-transaction-processing). In support of this design, the CALL Ledger provides an automated transaction censorship detector that is available on all `calld` servers, enabling all participants to see if censorship is affecting the network.

While a `calld` server is in sync with the network, the detector tracks all transactions that, in the view of the `calld` server, should have been accepted in the last round of [consensus](intro-to-consensus.html) and included in the last validated ledger. The detector issues log messages of increasing severity for transactions that have not been included in a validated ledger after several rounds of consensus.
...
1. The detector adds all transactions in a `calld` server's initial consensus proposal to the tracker.
...
3. The detector issues a [warning message](#example-warning-message) in the log for any transaction that remains in the tracker for 15 ledgers, surfacing it as a potentially censored transaction. The transaction's presence in the tracker at this time means that is has not been included in a validated ledger after 15 rounds of consensus. If the transaction remains in the tracker for another 15 ledgers, the detector issues another warning message in the log.
    ...
    If you see these messages in your `calld` server log, you should investigate why other servers are failing to include the transaction, starting with the assumption that the cause is more likely to be a [false positive](#potential-false-positives) (innocent bug) than malicious censorship.
...
- Your `calld` server is running a build with code that is different from the rest of the network. This may cause your `calld` server to apply transactions differently, resulting in false positives. While this type of false positive is unlikely, in general, it is crucial that you run the correct version of `calld`.

- Your `calld` server is out of sync with the network and has not yet realized it.

- `calld` servers in the network, including possibly your own server, are being impacted by a class of bug that causes `calld` servers to inconsistently relay transactions to other `calld` servers in the network.

    Currently, there are no known bugs that cause this unexpected behavior. However, if you see the impact of what you suspect is a bug, consider reporting it to the [Call Bug Bounty](http://www.callchain.live/bug-bounty/) program.
