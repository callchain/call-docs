# ledger_entry
[[Source]<br>](https://github.com/callchain/call-lib/blob/master/src/call/rpc/handlers/LedgerEntry.cpp "Source")

The `ledger_entry` method returns a single ledger object from the CALL Ledger in its raw format. See [ledger format][] for information on the different types of objects you can retrieve.
...
```
{
  ...
  "type": "account_root",
  "account_root": "cDVCAS1cnoMboELkQXxdmhJCiMscMm93U2",
...
5. `call_state` - Retrieve a [CallState object](callstate.html), which tracks a (non-CALL) currency balance between two accounts.
6. `check` - Retrieve a [Check object](check.html), which is a potential payment that can be cashed by its recipient. [New in: calld 1.0.0][]
7. `escrow` - Retrieve an [Escrow object](escrow-object.html), which holds CALL until a specific time or condition is met. [New in: calld 1.0.0][]
8. `payment_channel` - Retrieve a [PayChannel object](paychannel.html), which holds CALL for asynchronous payments. [New in: calld 1.0.0][]
9. `deposit_preauth` - Retrieve a [DepositPreauth object](depositpreauth-object.html), which tracks preauthorization for payments to accounts requiring [Deposit Authorization](depositauth.html). [New in: calld 1.1.0][]
...
| `call_state`          | Object                     | _(Optional)_ Object specifying the CallState (trust line) object to retrieve. The `accounts` and `currency` sub-fields are required to uniquely specify the CallState entry to retrieve. |
| `call_state.accounts` | Array                      | _(Required if `call_state` is specified)_ 2-length array of account [Address][]es, defining the two accounts linked by this [CallState object](callstate.html). |
| `call_state.currency` | String                     | _(Required if `call_state` is specified)_ [Currency Code][] of the [CallState object](callstate.html) to retrieve. |
| `binary`                | Boolean                    | _(Optional)_ If true, return the requested ledger object's contents as a hex string. Otherwise, return data in JSON format. The default is `false`. [Updated in: calld 1.2.0][New in: calld 1.2.0] |
...
<!-- TODO: we should add this ledger format link to calld-api-links.md. account_objects.md is also including this as a one-off.-->
