# log_level
[[Source]<br>](https://github.com/callchain/call-lib/blob/155fcdbcd0b4927152892c8c8be01d9cf62bed68/src/call/rpc/handlers/LogLevel.cpp "Source")

The `log_level` command changes the `calld` server's logging verbosity, or returns the current logging level for each category (called a _partition_) of log messages.

_The `log_level` method is an [admin method](admin-calld-methods.html) that cannot be run by unprivileged users._
...
calld log_level PathRequest debug
```
