...
[[Source]<br>](https://github.com/callchain/call-lib/blob/master/src/call/app/tx/impl/SetTrust.cpp "Source")
...
| `LimitAmount`.`currency` | String    | (Amount.currency) | The currency to this trust line applies to, as a three-letter [ISO 4217 Currency Code](http://www.xe.com/iso4217.php) or a 160-bit hex value according to [currency format](currency-formats.html). "CALL" is invalid. |
...
| tfSetNoCall   | 0x00020000 | 131072        | Blocks rippling between two trustlines of the same currency, if this flag is set on both. (See [No Call](rippling.html) for details.) If the [fix1578 amendment][] is enabled, a transaction that uses this flag and cannot enable NoCall fails with the result code `tecNO_PERMISSION`. If the amendment is not enabled, the transaction can result in `tesSUCCESS` (making any other changes it can) even if it cannot enable NoCall on the trust line. |
| tfClearNoCall | 0x00040000 | 262144        | Clears the No-Rippling flag. (See [NoCall](rippling.html) for details.) |
