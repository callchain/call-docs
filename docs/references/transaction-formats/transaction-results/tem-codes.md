...
| `temBAD_FEE`                  | The transaction improperly specified its `Fee` value, for example by listing a non-CALL currency or some negative amount of CALL. |
...
| `temBAD_OFFER`                | The [OfferCreate transaction][] specifies an invalid offer, such as offering to trade CALL for itself, or offering a negative amount. |
| `temBAD_PATH`                 | The [Payment transaction][] specifies one or more [Paths](paths.html) improperly, for example including an issuer for CALL, or specifying an account differently. |
...
| `temBAD_SEND_CALL_LIMIT`     | The [Payment transaction][] used the [tfLimitQuality](payment.html#limit-quality) flag in a direct CALL-to-CALL payment, even though CALL-to-CALL payments do not involve any conversions. |
| `temBAD_SEND_CALL_MAX`       | The [Payment transaction][] included a `SendMax` field in a direct CALL-to-CALL payment, even though sending CALL should never require SendMax. (CALL is only valid in SendMax if the destination `Amount` is not CALL.) |
| `temBAD_SEND_CALL_NO_DIRECT` | The [Payment transaction][] used the [tfNoDirectCall](payment.html#payment-flags) flag for a direct CALL-to-CALL payment, even though CALL-to-CALL payments are always direct. |
| `temBAD_SEND_CALL_PARTIAL`   | The [Payment transaction][] used the [tfPartialPayment](partial-payments.html) flag for a direct CALL-to-CALL payment, even though CALL-to-CALL payments should always deliver the full amount. |
| `temBAD_SEND_CALL_PATHS`     | The [Payment transaction][] included `Paths` while sending CALL, even though CALL-to-CALL payments should always be direct. |
...
| `temREDUNDANT_SEND_MAX`      | [Removed in: calld 0.28.0][] |
| `temCALL_EMPTY`             | The [Payment transaction][] includes an empty `Paths` field, but paths are necessary to complete this payment. |
