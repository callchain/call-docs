...
* `unknownCmd` - The request does not contain a [command](calld-api.html) that the `calld` server recognizes.
...
* `noNetwork` - The server is having trouble connecting to the rest of the CALL Ledger peer-to-peer network (and is not running in stand-alone mode).
