...
## {{n.next()}}. Connect to the CALL Ledger
...
$("#connect-button").click((event) => {
  socket = new WebSocket('wss://s1.callchain.live:5020')
  socket.addEventListener('open', (event) => {
    // This callback runs when the connection is open
...
<input type="text" class="form-control" id="subscribe_address" value="cUCzEr6jrEyMpjhs4wSdQdz4g8Y382NxfM">
...
- The **`transaction.TransactionType` field** is the type of transaction. The transaction types that can possibly deliver currency to an account are as follows:
    ...
    - **[EscrowFinish transactions][]** can deliver CALL by finishing an [Escrow](escrow.html) created by a previous [EscrowCreate transaction][]. Look at the metadata of the **EscrowFinish transaction** to see which account received CALL from the escrow and how much.
    ...
    - **[PaymentChannelClaim transactions][]** can deliver CALL from a [payment channel](payment-channels.html). Look at the metadata to see which accounts, if any, received CALL from the transaction.

    - **[PaymentChannelFund transactions][]** can return CALL from a closed (expired) payment channel to the sender.
...
The following sample code looks at transaction metadata of all the above transaction types to report how much CALL an account received:
...
<script type="application/javascript">
function CountCALLDifference(affected_nodes, address) {
  ...
  // any CALL balance changes combined with the transaction cost.
    ...
    if ((affected_nodes[i].hasOwnProperty("ModifiedNode"))) {
      ...
      if (ledger_entry.LedgerEntryType === "AccountRoot" &&
        ...
        if (!ledger_entry.PreviousFields.hasOwnProperty("Balance")) {
          writeToConsole("#monitor-console-read", "CALL balance did not change.")
        }
        ...
        const call_amount = diff_in_drops.div(1e6)
        if (call_amount.gte(0)) {
          writeToConsole("#monitor-console-read", "Received " + call_amount.toString()+" CALL.")
          return
        } else {
          writeToConsole("#monitor-console-read", "Spent " + call_amount.abs().toString() + " CALL.")
          return
        }
      }
      ...
      if (ledger_entry.LedgerEntryType === "AccountRoot" &&
        ...
        const call_amount = balance_drops.div(1e6)
        writeToConsole("#monitor-console-read", "Received " + call_amount.toString() + " CALL (account funded).")
        return
      }
...
function CountCALLReceived(tx, address) {
  ...
  if (tx.transaction.TransactionType === "Payment") {
    ...
    if (typeof tx.meta.delivered_amount === "string") {
      ...
      const call_amount = amount_in_drops.div(1e6)
      writeToConsole("#monitor-console-read", "Received " + call_amount.toString() + " CALL.")
      return
    } else {
      writeToConsole("#monitor-console-read", "Received non-CALL currency.")
      return
    }
  } else if (["PaymentChannelClaim", "PaymentChannelFund", "OfferCreate",
          "CheckCash", "EscrowFinish"].includes(
          tx.transaction.TransactionType)) {
    CountCALLDifference(tx.meta.AffectedNodes, address)
  } else {
