...
The `RequireDest` setting (`requireDestinationTag` in CallAPI) is designed to prevent customers from sending payments to your address if they forgot the [destination tag](source-and-destination-tags.html) that identifies who should be credited for the payment. When enabled, the CALL Ledger rejects any payment to your address that does not specify a destination tag.

The following is an example of using a locally-hosted `calld`'s [submit method][] to send an AccountSet transaction to enable the `RequireDest` flag:
