...
Financial institutions and other services using the CALL Ledger should use the best practices described here to make sure that transactions are validated or rejected in a verifiable and prompt way.  You should submit transactions to trusted (locally operated) `calld` servers.

The best practices detailed in this document allow applications to submit transactions to the CALL Ledger while achieving:
...
The CALL Ledger provides several APIs for submitting transactions, including [`calld` API](calld-api.html), and [CallAPI](callapi-reference.html).  Regardless of the API used, the transaction is applied to the ledger as follows.
...
**Note:** When submitting a transaction via `calld`, a successful status code returned from a submit command indicates the `calld` server has received the candidate transaction. The transaction may or may not be applied to a validated ledger.
...
While applying transactions, `calld` servers use the *last validated ledger*, a snapshot of the ledger state based on transactions the entire network has validated.  The process of consensus and validation apply a set of new transactions to the last validated ledger in canonical order, resulting in a new validated ledger.  This new validated ledger instance and the ones that preceded it form the ledger history.
...
`LastLedgerSequence` is an optional [parameter of all transactions](transaction-common-fields.html).  This instructs the CALL Ledger that a transaction must be validated on or before a specific ledger instance.  The CALL Ledger never includes a transaction in a ledger instance whose sequence number is higher than the transaction's `LastLedgerSequence` parameter.
...
Applications using `calld` APIs should explicitly specify a `LastLedgerSequence` when submitting transactions.

CallAPI uses the `maxLedgerVersion` field of [Transaction Instructions](callapi-reference.html#transaction-instructions) to specify the `LastLedgerSequence`.  CallAPI automatically provides an appropriate value by default.  You can specify `maxLedgerVersion` as `null` to intentionally omit `LastLedgerSequence`, in case you want a transaction that can be executed after an unlimited amount of time (this is strongly discouraged).
...
Alternatively, you can look up the status of the transaction using a different `calld` server that already has the needed ledger history, such as Call's full-history servers at `s1.callchain.live`. Only use a server you trust for this purpose. A malicious server could be programmed to provide false information about the status and outcome of a transaction.
...
1. The [`calld` API](calld-api.html)
2. [CallAPI](callapi-reference.html)
3. Any number of other software APIs layered on top of `calld`


### calld - Submitting and Verifying Transactions

#### Determine the Account Sequence

`calld` provides the [account_info method][] to learn an account's sequence number in the last validated ledger.
...
`calld` provides the [sign method][] to prepare a transaction for submission.  This method requires an account secret, which should only be passed to trusted `calld` instances.  This example issues 10 FOO (a made-up currency) to another CALL Ledger address.
...
`calld` provides the [submit method][], allowing us to submit the signed transaction.  This uses the `tx_blob` parameter that was returned by the `sign` method.
...
The `txnNotFound` result code occurs in cases where the transaction is not included in any ledger.  However, it could also occur when a `calld` instance does not have a complete ledger history, or if the transaction has not yet propagated to the `calld` instance.  Applications should make further queries to determine how to react.
...
Finally the server may show one or more gaps in the transaction history. The `completed_ledgers` field shown in the response above indicates that ledgers 10256383 through 10256411 are missing from this calld instance.  Our example transaction can only appear in ledgers 10268597 - 10268600 (based on when it was submitted and `LastLedgerSequence`), so the gap shown here is not relevant.  However, if the gap indicated a ledger in that range was missing, then an application would need to query another calld server (or wait for this one to retrieve the missing ledgers) to determine that a `txnNotFound` result is immutable.
...
- [Overview of CALL Ledger Consensus Process](consensus.html)
