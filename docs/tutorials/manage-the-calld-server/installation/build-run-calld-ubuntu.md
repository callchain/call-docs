# Build and Run calld on Ubuntu

`calld` is the core peer-to-peer server that manages the CALL Ledger. A `calld` server can connect to a network of peers, relay cryptographically signed transactions, and maintain a local copy of the complete shared global ledger.

For an overview of `calld`, see [Operating calld Servers](install-calld.html).

Use these instructions to build a `calld` executable from source version 1.2.0 or higher on Ubuntu Linux 16.04 or higher. These instructions were tested on Ubuntu 16.04 LTS.

For information about building `calld` for other platforms, see [Builds](https://github.com/callchain/call-lib/tree/develop/Builds) in the `calld` GitHub repository.


## Prerequisites

Before you compile or install `calld`, you must meet the [System Requirements](system-requirements.html).

## 1. Build `calld`

These instructions use Ubuntu's APT (Advanced Packaging Tool) to install the software prerequisites you need to build and run `calld`.
...
4. Install CMake.
    ...
    Version 1.3.1 of `calld` requires CMake 3.9.0 or higher. For the purposes of this tutorial, we used CMake 3.13.3, which was the latest version available at the time of writing.
...
5. Compile Boost.
    ...
    Version 1.3.1 of `calld` requires Boost version 1.70.0 or higher. Because Boost version 1.70.0 or higher isn't available in the Ubuntu 18.04 (or 16.04) software repositories, you must compile it yourself.

    If you have previously built Boost 1.70.0 for `calld` and configured the `BOOST_ROOT` environment variable, you can skip these steps.
...
6. From a working directory, get the `calld` source code. The `master` branch has the latest released version.

        git clone https://github.com/callchain/call-lib.git
        cd calld
        git checkout master

7. Check the commit log to be sure you're compiling the version you intend to. The most recent commit should be signed by a well-known Call developer and should set the version number to the latest released version. For example:

        $ git log -1

        commit e1adbd7ddd5dfa9f2a9791aa3c0fcc1fdb4e8236
        Author: Manoj doshi <mdoshi@call.com>
...
8. If you previously built, or (more importantly) tried and failed to build `calld`, you should delete the `my_build/` directory (or whatever you named it) to start clean before moving on to the next step. Otherwise, you may get unexpected behavior, like a `calld` executable that crashes due to a segmentation fault (segfault).

    If this is your first time building `calld` 1.0.0 or higher, you won't have a `my_build/` directory and can move on to the next step.

9. Use CMake to build a `calld` binary executable from source code. The result will be a `calld` binary executable in the `my_build` directory.

      1. Generate the build system. Builds should be performed in a directory that is separate from the source tree root. In this example, we'll use a `my_build` directory that is a subdirectory of `calld`.

              mkdir my_build
              cd my_build
              cmake ..

          **Tip:** The default build includes debugging symbols, which can be useful for development but are inefficient in production. To build `calld` for use on production servers, add the `-DCMAKE_BUILD_TYPE=Release` flag when running the `cmake` command.

      2. Build the `calld` binary executable. This may take about 30 minutes, depending on your hardware specs.

              cmake --build .

10. _(Optional)_ Run `calld` unit tests. If there are no test failures, you can be fairly certain that your `calld` executable compiled correctly.
...
Complete the following configurations that are required for `calld` to start up successfully. All other configuration is optional and can be tweaked after you have a working server.

1. Create a copy of the example config file (assumes you're in the `calld` folder already). Saving the config file to this location enables you to run `calld` as a non-root user (recommended).

        mkdir -p ~/.config/call
        cp cfg/calld-example.cfg ~/.config/call/calld.cfg

2. Edit the config file to set necessary file paths. The user you plan to run `calld` as must have write permissions to all of the paths you specify here.
      ...
      3. Set the `[debug_logfile]` to a path where `calld` can write logging information.

3. Copy the example `validators.txt` file to the same folder as `calld.cfg`:

        cp cfg/validators-example.txt ~/.config/call/validators.txt

    **Warning:** Call has designed a [decentralization plan](http://www.callchain.live/blog/2017/decent-strategy-update.html) with maximum safety in mind. During the transition, you *should not* modify the `validators.txt` file except as recommended by Call. Even minor modifications to your validator settings could cause your server to diverge from the rest of the network and report out of date, incomplete, or inaccurate data. Acting on such data can cause you to lose money.


## 3. Run `calld`

To run your stock `calld` server from the executable you built, using the configurations you defined:
```
cd my_build
./calld
```
...
Once you've run `calld`, here are excerpts of what you can expect to see in your terminal.

```
Loading: "/home/ubuntu/.config/call/calld.cfg"
...
* Now that you have a stock `calld` server running, you may want to consider running it as a validating server. For information about validating servers and why you might want to run one, see the [calld Setup Tutorial](install-calld.html).

* For information about communicating with your `calld` server using the `calld` API, see the [`calld` API reference](calld-api.html).

* As a development best practice, you may want to build a `calld` `.deb` file. For more information, see _Ubuntu Packaging Guide_: [Packaging New Software](http://packaging.ubuntu.com/html/packaging-new-software.html).

* You may also want to install a `systemd` unit. For more information, see [systemd for Upstart Users](https://wiki.ubuntu.com/SystemdForUpstartUsers). You can use the [official `calld` system unit file](https://github.com/callchain/call-lib-package-builder/blob/staging/rpm-builder/calld.service) or modify it to suit your needs.
