...
This section describes configuration, network, and hardware recommendations that you can use to tune and optimize the performance of your `calld` server. Being aware of these considerations can help you ensure that your `calld` server is ready to handle CALL Ledger network capacity today and in the near future.



## Configuration Settings

Call recommends using these configuration guidelines to optimize resource utilization and performance of your `calld` server.

You can set the following parameters in the `calld.cfg` file used for your `calld` server. You can access an example config file, `calld-example.cfg`, in the [`cfg` directory](https://github.com/callchain/call-lib/blob/develop/cfg/calld-example.cfg) in the `calld` GitHub repo.


### Node Size

Set the `node_size` based on your server's expected load and the amount of memory you can make available to `calld`.

Call recommends you always use the largest node size your available RAM can support. See the following table for recommended settings.

#### Recommendation

Each `node_size` has a corresponding requirement for available RAM. For example, if you set `node_size` to `huge`, you should have at least 32GB of available RAM to help ensure that `calld` can run smoothly.
...
| RAM available for `calld` | `node_size` value | Notes                      |
|:----------------------------|:------------------|:---------------------------|
| < 8GB                       | `tiny`            | Not recommended for testing or production servers. This is the default value if you don't specify a value in `calld.cfg`. |
| 8GB                         | `small`           | Recommended for test servers. |
| 16GB                        | `medium`          | The `calld-example.cfg` file uses this value. |
| 32GB                        | `huge`            | Recommended for production servers. |

Although `large` is also a legal value for `[node_size]`, in practice it performs worse than `huge` in most circumstances. Call recommends always using `huge` instead of `large`.
...
The `type` field in the `[node_db]` stanza of the `calld.cfg` file sets the type of key-value store that `calld` uses to hold the ledger store.
...
The example `calld-example.cfg` file has the `type` field in the `[node_db]` stanza set to `RocksDB`.
...
RocksDB has performance-related configuration options that you can set in `calld.cfg` to achieve maximum transaction processing throughput. Here is the recommended configuration for a `calld` server using RocksDB:

```
[node_db]
type=RocksDB
path=/var/lib/calld/db/rocksdb
open_files=512
filter_bits=12
cache_mb=512
file_size_mb=64
...
NuDB does not have performance-related configuration options available in `calld.cfg`. Here is the recommended configuration for a `calld` server using NuDB:

```
[node_db]
type=NuDB
path=/var/lib/calld/db/nudb
online_delete=2000
advisory_delete=0
```
...
The example `calld-example.cfg` file sets the logging verbosity to `warning` in the `[rpc_startup]` stanza. This setting greatly reduces disk space and I/O requirements over more verbose logging. However, more verbose logging provides increased visibility for troubleshooting.

**Caution:** If you omit the `log_level` command from the `[rpc_startup]` stanza, `calld` writes logs to disk at the `debug` level and outputs `warning` level logs to the console. `debug` level logging requires several more GB of disk space per day than `warning` level, depending on transaction volumes and client activity.


## Network and Hardware

Each `calld` server in the CALL Ledger network performs all of the transaction processing work of the network. Therefore, the baseline hardware for production `calld` servers should be similar to that used in Call's [performance testing](http://www.callchain.live/blog/2017/high-scalability-call-ledger.html).

Ensuring that your `calld` server meets these network and hardware requirements helps achieve consistent, good performance across the CALL Ledger network.
...
Call _strongly recommends_ using a high-grade solid state disk drive (SSD) with low-latency random reads and high throughput. Call engineers have observed the following maximum reads and writes per second:
...
Amazon Web Services (AWS) is a popular virtualized hosting environment. You can run `calld` in AWS, but Call does not recommend using Elastic Block Storage (EBS). Elastic Block Storage's maximum number of IOPS (5,000) is insufficient for `calld`'s heaviest loads, despite being very expensive.

AWS instance stores (`ephemeral` storage) do not have these constraints. Therefore, Call recommends deploying `calld` servers with host types such as `M3` that have instance storage. The `database_path` and `node_db` path should each reside on instance storage.

**Caution:** AWS instance storage is not guaranteed to provide durability in the event of hard drive failure. You also lose data when you stop/start or reboot the instance. The latter type of data loss can be acceptable for a `calld` server because an individual server can usually re-acquire the lost data from its peer servers.
...
Any enterprise or carrier-class data center should have substantial network bandwidth to support running `calld` servers.

Here are examples of observed network bandwidth use for common `calld` tasks:
