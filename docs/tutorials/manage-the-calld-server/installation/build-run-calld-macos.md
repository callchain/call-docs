# Build and Run calld on macOS

At this time, Call doesn't recommend using the macOS platform for `calld` production use. For production, consider using the [Ubuntu platform](install-calld-on-ubuntu-with-alien.html), which has received the highest level of quality assurance and testing.

That said, macOS is suitable for many development and testing tasks. `calld` has been tested for use with macOS up to 10.13 High Sierra.

For development purposes Call recommends running `calld` as your own user, not using `sudo`.
...
0. Install Boost 1.70.0 or higher. `calld` 1.3.1 is compatible with Boost 1.70.0 and higher. The latest version of Boost available in the Homebrew repositories isn't new enough, so you must install Boost manually:
...
0. Clone the `calld` source code into your desired location and access the `calld` directory. To do this, you'll need to set up Git (installed earlier using Homebrew) and GitHub. For example, you'll need to create a GitHub account and set up your SSH key. For more information, see [Set up git](https://help.github.com/articles/set-up-git/).
...
0. By default, cloning puts you on the `develop` branch. Use this branch if you are doing development work and want the latest set of untested features.
      ...
      Or, you can checkout one of the tagged releases listed on [GitHub](https://github.com/callchain/call-lib/releases).

0. In the `calld` directory you just cloned, create your build directory and access it. For example:

        $ mkdir my_build
        $ cd my_build

0. Build `calld`. This could take about 5 minutes, depending on your hardware specs.
...
0. `calld` requires the `calld.cfg` config file to run. You can find an example config file, `calld-example.cfg` in `calld/cfg`. Make a copy and save it as `calld.cfg` in a location that enables you to run `calld` as a non-root user. Access the `calld` directory and run:

        $ mkdir -p $HOME/.config/call
        $ cp cfg/calld-example.cfg $HOME/.config/call/calld.cfg

0. Edit `calld.cfg` to set necessary file paths. The user you plan to run `calld` as must have write permissions to all of the paths you specify here.
      ...
      * Set the `[debug_logfile]` to a path where `calld` can write logging information.

      These are the only configurations required for `calld` to start up successfully. All other configuration is optional and can be tweaked after you have a working server. For more information, see [Additional Configurations](#additional-configuration).

0. `calld` requires the `validators.txt` file to run. You can find an example validators file, `validators-example.txt`, in `calld/cfg/`. Make a copy and save it as `validators.txt` in the same folder as your `calld.cfg` file. Access the `calld` directory and run:

        $ cp cfg/validators-example.txt $HOME/.config/call/validators.txt

      **Warning:** Call has designed a decentralization plan with maximum safety in mind. During the transition, you should not modify the `validators.txt` file except as recommended by Call. Even minor modifications to your validator settings could cause your server to diverge from the rest of the network and report out of date, incomplete, or inaccurate data. Acting on such data can cause you to lose money.

0. Access your build directory, `my_build` for example, and start the `calld` service.
...
For information about `calld` log messages, see [Understanding Log Messages](understanding-log-messages.html).
...
- [Install calld on Ubuntu Linux](install-calld-on-ubuntu.html) (Pre-built binary on Ubuntu for production use)
- [Install calld on Red Hat Enterprise Linux](install-calld-on-centos-rhel-with-yum.html) (Pre-built binary on RHEL or CentOS for production use)
