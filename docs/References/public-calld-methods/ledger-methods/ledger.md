# ledger
[[Source]<br>](https://github.com/callchain/call-lib/blob/master/src/call/rpc/handlers/LedgerHandler.cpp "Source")
...
| `binary`       | Boolean                    | _(Optional)_ If `true`, and `transactions` and `expand` are both also `true`, return transaction information in binary format (hexadecimal string) instead of JSON format. [New in: calld 0.28.0][] |
...
| `ledger.close_time`            | Integer | The time this ledger was closed, in [seconds since the Call Epoch][] |
...
| `ledger.total_coins`           | String  | Total number of CALL drops in the network, as a quoted integer. (This decreases as transaction costs destroy CALL.) |
...
| `fee`               | String           | _(May be omitted)_ The [Transaction Cost](transaction-cost.html) of this transaction, in [drops of CALL][]. |
...
| `max_spend_drops`   | String           | _(May be omitted)_ The maximum amount of [CALL, in drops][], this transaction could potentially send or destroy. |
...
<!-- TODO: we should add this fee levels link to calld-api-links.md. server_state.md is also including this as a one-off.-->
