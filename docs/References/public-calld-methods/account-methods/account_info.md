# account_info
[[Source]<br>](https://github.com/callchain/call-lib/blob/master/src/call/rpc/handlers/AccountInfo.cpp "Source")

The `account_info` command retrieves information about an account, its activity, and its CALL balance. All information retrieved is relative to a particular version of the ledger.
...
calld account_info cDVCAS1cnoMboELkQXxdmhJCiMscMm93U2 true
```
...
| `strict`       | Boolean                    | (Optional, defaults to False) If set to True, then the `account` field only accepts a public key or CALL Ledger address. |
...
| `queue`        | Boolean                    | _(Optional)_ If `true`, and the [FeeEscalation amendment][] is enabled, also returns stats about queued transactions associated with this account. Can only be used when querying for the data from the current open ledger. [New in: calld 0.33.0][] |
| `signer_lists` | Boolean                    | _(Optional)_ If `true`, and the [MultiSign amendment][] is enabled, also returns any [SignerList objects](signerlist.html) associated with this account. [New in: calld 0.31.0][] |
...
| `max_spend_drops_total` | String  | (May be omitted) Integer amount of [drops of CALL][] that could be debited from this address if every transaction in the queue consumes the maximum amount of CALL possible. |
...
| `fee`             | String  | The [Transaction Cost](transaction-cost.html) of this transaction, in [drops of CALL][]. |
...
| `max_spend_drops` | String  | The maximum amount of [CALL, in drops][], this transaction could send or destroy. |
