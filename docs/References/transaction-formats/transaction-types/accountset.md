...
[[Source]<br>](https://github.com/callchain/call-lib/blob/f65cea66ef99b1de149c02c15f06de6c61abf360/src/call/app/transactors/SetAccount.cpp "Source")

An AccountSet transaction modifies the properties of an [account in the CALL Ledger](accountroot.html).
...
You can put any domain in your account's `Domain` field. To prove that an account and domain belong to the same person or business, Call recommends establishing a "two-way link":

- Accounts you own should have a domain you own in the `Domain` field.
- On a website at that domain, host an [call-ledger.toml file](call-ledger-toml.html) listing accounts you own, and optionally other information about how you use the CALL Ledger.
...
| asfDefaultCall | 8             | lsfDefaultCall          | Enable [rippling](rippling.html) on this account's trust lines by default. [New in: calld 0.27.3][] |
...
| asfDisallowCALL   | 3             | lsfDisallowCALL            | CALL should not be sent to this account. (Enforced by client applications, not by `calld`) |
...
| tfDisallowCALL     | 0x00100000 | 1048576       | asfDisallowCALL (SetFlag)    |
| tfAllowCALL        | 0x00200000 | 2097152       | asfDisallowCALL (ClearFlag)  |
...
Incoming transactions with unclear purposes may be an inconvenience for financial institutions, who would have to recognize when a customer made a mistake, and then potentially refund accounts or adjust balances depending on the mistake. The `asfRequireDest` and `asfDisallowCALL` flags are intended to protect users from accidentally sending funds in a way that is unclear about the reason the funds were sent.
...
You can protect against unwanted incoming payments for non-CALL currencies by not creating trust lines in those currencies. Since CALL does not require trust, the `asfDisallowCALL` flag is used to discourage users from sending CALL to an account. However, this flag is not enforced in `calld` because it could potentially cause accounts to become unusable. (If an account did not have enough CALL to send a transaction that disabled the flag, the account would be completely unusable.) Instead, client applications should disallow or discourage CALL payments to accounts with the `asfDisallowCALL` flag enabled.
